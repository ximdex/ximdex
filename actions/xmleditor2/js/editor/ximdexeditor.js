...
function XimdocEditor(options) {
    ...
    this.initialize = function (callback) {
        ...
        // We think fileRequest is contained in fileRequestAll.
            ...
            var confirmCallback = {
                'yes': function () {
                    ...
                    new AjaxRequest(this._baseURL + '&ajax=json&method=recoverTmpFile&view=' + this.getView(), {
                        ...
                        onComplete: function (req, json) {
                            if (json.result === true) {
                                this.fileRequest('_xmlDom', this._baseURL + '&ajax=json&method=getXmlFile', function () {
                                    this.tools.editorviewtool.setView(this.getView());
                                    this._hasTmp = false;
                                }.bind(this));
...
this.afterUpdateContent = function (options) {
    ...
    var content = this.getXimDocument().saveXML({
        asString: true,
        hideXimlets: true
    });
    ...
    com.ximdex.ximdex.editors.ValidateHandler(kupu.getBaseURL(), encodedObject, {
        onComplete: function (r, json) {
            ...
            if (!json.valid) {
                ...
                msg += "Document not valid";
                if (json.errors) {
                    var l = json.errors.length;
                    for (var i = 0; i < l; i++) {
                        msg += '<br /> -' + json.errors[i];
                    }
                }
            } else {
            ...
            $('#kupu-schemavalidator-button').children('span.tooltip').html(msg);
        },
        onError: function (req) {
