...
<div>
	...
	<div class="action_content">
		...
		<form method="post" id="delete_rel" class="delete_rel" action="{$action_delete}">
			...
			<fieldset>
				<input type="checkbox" name="recursive" id="{$id_node}_recursive_delete" />
				<label for="{$id_node}_recursive_delete"> {t}Disassociate recursively{/t}.</label>
		...
		{/if} {if count($linkable_ximlets) > 0 }
		<form id="create_rel" class="create_rel" action="{$action_create}" method="post">
			<fieldset>
				<h3>{t}Available Ximlets{/t}</h3>
				<div class="copy_options">
					<input type="hidden" id="id_node" name="id_node" value="{$id_node}" /> {foreach from=$linkable_ximlets item=ximlet}
					<div>
			...
			<fieldset>
				<input type="checkbox" name="recursive" id="{$id_node}_recursive_add" />
				<label for="{$id_node}_recursive_add"> {t}Associate recursively{/t}.</label>
				<p>{t}If the current section/server has subfolders, the association will be created for them too{/t}.</p>
			</fieldset>
			<fieldset class="buttons-form">
				{button label="Associate" class='validate btn main_action' }{*message="Would you like to associate this section with the
				ximlet?"*}
			</fieldset>
		</form>
		{else}
