angular.module('ximdex.main.controller').controller 'SearchTreeModalCtrl', [
    ...
    "nodetypesAllowedToShow", "nodetypesAllowedToSelect", "title",
    ($scope, $modalInstance, xTranslate, $window, $http, xUrlHelper, xMenu, $document, $timeout,
     $q, nodetypesAllowedToShow, nodetypesAllowedToSelect, title) ->
#Nodes for project tab
        ...
        $scope.ok = () ->
            if $scope.selectedNodes?.length == 1 && $scope.selectedNodes[0].nodetypeid in $scope.nodetypesAllowedToSelect
                $modalInstance.close($scope.selectedNodes[0])
            else
                $scope.error = true
