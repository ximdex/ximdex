...
(function (X) {
    ...
    X.Tour = Object.xo_create({
        ...
        endTour: function () {
            ...
            if (disabledTour) {
                $.ajax({
                    ...
                    error: function (data) {
                        console.log("ERROR","Al relizar la petitición para deshabilitar Xtour")
                    }
                });
            }
        },
        ...
        showTooltip: function (firstTime) {
            ...
            var myFunction = function () {
                ...
                $("div#tourcontrols p.step", that.scope)[0].innerHTML = that._getStepText();
                $('.tour_mark', that.scope).toggleClass('tour_mark');
...
//	    $(step_config.name,this.scope).toggleClass('tour_mark');
                ...
                $('body', that.scope).prepend($tooltip);
                ...
                } else if (!step_config.scope && that.scope != window.document) {
                    extra_top = $("iframe.action_iframe", that.scope).offset().top;
                    extra_left = $("iframe.action_iframe", that.scope).offset().left;
                }
                    ...
                    $('html, body').stop()
                        .animate({
                            ...
                            if (that.autoplay) {
                                ...
                                that.showtime = setTimeout(that.nextStep, step_config.time);
                            }
                ...
                if (step_config.title)
                    $("div.tourcontrols p.tourtitle", that.scope)[0].innerHTML = step_config.title;

                if (firstTime) {
                    var newtop = ($(window).height() - $('#tourcontrols', this.scope).outerHeight() - 10
                        + $(window).scrollTop()) + "px";
            ...
            if (step_config.timeToWait) {
                window.setTimeout(myFunction, step_config.timeToWait);
            } else {
                myFunction();
            }
        ...
        showControls: function (command) {
            ...
            $.getJSON(
                ...
                function (data) {
                    ...
                    this.restartEvents();
                    $('#tourcontrols', this.scope).css("top", (($(window).height() - $('#tourcontrols', this.scope).outerHeight()) / 2) + $(window).scrollTop() + "px");
