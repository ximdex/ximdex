...
<h2>{t}Document preview{/t} {$name}</h2>
	<fieldset class="action-controls">
						...
						<a id="prevdoc-button" href="{$_URL_ROOT}/?action=prevdoc&nodeid={$id_node}" class="btn main_action ui-state-default ui-corner-all button submit-button ladda-button"><span class="ladda-label">{t}View in a new window{/t}</span></a>

	</fieldset>
</div>
<div class="content_container prevdoc">
	<iframe id="preview{$id_node}" src="{$_URL_ROOT}/?action=prevdoc&nodeid={$id_node}&channel=10001" height="100%" width="100%">
