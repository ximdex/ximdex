...
class Scheduler
{
    public static function start($global_execution = true)
    {
        ...
        do {
            ...
            if (!$activeAndEnabledServers || count($activeAndEnabledServers) == 0) {
                ...
                while ($batchProcess) {
                    ...
                    if ($cycles >= MAX_NUM_CICLOS_SCHEDULER) {
                        ...
                        // Exceding max. cycles...
                        $msg = sprintf("Exceding max. cycles (%d > %d). Exiting scheduler", $cycles, MAX_NUM_CICLOS_SCHEDULER);
                        $syncStatObj->create(null, null, null, null, null, __CLASS__, __FUNCTION__, __FILE__, __LINE__, "INFO", 8, $msg);
                        Logger::info($msg);
            ...
            if ($global_execution) {
                if ($voidCycles > MAX_NUM_CICLOS_VACIOS_SCHEDULER) {
                    Logger::info(sprintf("Exceding max. cycles (%d > %d). Exit scheduler", $voidCycles, MAX_NUM_CICLOS_VACIOS_SCHEDULER));
                    break;
                }
            } else {
        ...
        $msg = sprintf("Exceding max. cycles (%d > %d). Exit scheduler", $cycles, MAX_NUM_CICLOS_VACIOS_SCHEDULER);
        $syncStatObj->create(null, null, null, null, null, __CLASS__, __FUNCTION__, __FILE__, __LINE__, "INFO", 8, $msg);
        Logger::info($msg);
        $mutex->release();
    }
