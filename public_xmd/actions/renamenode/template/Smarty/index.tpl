...
<form method="post" name="ren_form" id="ren_form" action="{$action_url}">
	<input type="hidden" name="id_node" value="{$id_node}" /> {include file="actions/components/title_Description.tpl"}
		...
		<div class="row tarjeta">
			...
			<div class="small-12 columns">
				<div class="input">
					...
					<input type="hidden" name="name" id="name" value="{$name}" /> {else}
			...
			{if $id_nodetype==5078}
			<div class="small-12 columns">
				<input type="radio" name="schema_type" value="generic_schema" {if $schema_type=='generic_schema' || $schema_type=='' } checked="checked"
				 {/if} id="generic-scheme_{$id_node}" class="hidden-focus">
				<label for="generic-scheme_{$id_node}" class="icon radio-label">{t}Generic schema{/t}</label>
			</div>
			<div class="small-12 columns">
				<input type="radio" name="schema_type" value="metadata_schema" {if $schema_type=='metadata_schema' } checked="checked" {/if}
				 id="metadata-scheme_{$id_node}" class="hidden-focus">
				<label for="metadata-scheme_{$id_node}" class="icon radio-label">{t}Metadata schema{/t}</label>
			</div>
			{/if} {if !empty($valid_pipelines)}
			...
			<div class="small-12 columns">
				...
				<div class="languages-section">
					<input type="checkbox" name="languages[]" id="lang_{$language_info.Name}" value="{$language_info.IdLanguage}" class="hidden-focus"
					/>
			...
			<div class="small-12 columns">
				...
				<fieldset class="buttons-form">
					{* {if $id_nodetype!=5078 & $id_nodetype!=5048} {button label="Check dependencies" class="open_report btn"} {/if} *} {button
					label="Modify" class='validate btn main_action'} {*message="Are you sure you want to change the name?"*}
				</fieldset>
			</div>
